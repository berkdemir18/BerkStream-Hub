...
class WFilmIzle : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        var home: List<SearchResponse>? =
            document.select("div.movie-preview-content").mapNotNull { it.toMainPageResult() }
        if (request.name == "En Son Eklenenler") {
            home = document.selectFirst("div.fix-film_item")?.select("div.movie-preview-content")
    ...
    private fun Element.toMainPageResult(): SearchResponse {
        ...
        val score = this.selectFirst("span.icon-star")?.text()?.replace("IMDb", "")?.trim() ?: ""
        Log.d("WFI", "score: $score")
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val orgTitle =
            document.selectFirst("div.title h1")?.text()?.replace(" izle", "")?.trim() ?: ""
        ...
        Log.d("WFI", "tags: $tags")
        val rating =
            document.select("div.imdb").last()?.text()?.replace("IMDb Puanı:", "")?.split("/")
                ?.first()?.trim()
    ...
    override suspend fun loadLinks(
        ...
        if (iframe?.contains("hdplayersystem") == true) {
            val json = app.post(
                "https://hdplayersystem.live/player/index.php?data=$hash&do=getVideo",
                headers =
                    mapOf(
            ...
            val jsonData =
                ObjectMapper().readValue(json.document.body().text(), IframeResponse::class.java)
